...
export const HeroSlider = () => {
  ...
  return (
    <section className="relative min-h-[85vh] overflow-hidden hero-gradient">
      ...
      {/* Content */}
      <div className="relative container mx-auto px-4 py-24 lg:py-32 min-h-[85vh] flex items-center">
        <div className="max-w-2xl">
          {/* Headline */}
          <h1
            key={`headline-${currentSlide}`}
            className="font-heading text-4xl md:text-5xl lg:text-6xl font-bold text-primary-foreground mb-2 animate-fade-in leading-tight"
          >
            {slides[currentSlide].headline}
          </h1>
          <h2
            key={`subline-${currentSlide}`}
            className="font-heading text-3xl md:text-4xl lg:text-5xl font-bold text-primary-foreground/90 mb-6 animate-fade-in-delay-1"
          >
          ...
          <p
            ...
            className="text-lg text-primary-foreground/75 mb-8 max-w-xl animate-fade-in-delay-2 leading-relaxed"
          >
            {slides[currentSlide].description}
          </p>

          {/* Certifications */}
          <div className="flex flex-wrap items-center gap-4 mb-10 animate-fade-in-delay-2">
            {certifications.map((cert, index) => (
              <span key={cert} className="flex items-center gap-1 text-primary-foreground/80 text-sm">
                {index > 0 && <span className="text-primary-foreground/40 mr-3">|</span>}
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4">
          <button
            onClick={handlePrev}
            className="p-3 rounded-full bg-primary-foreground/10 hover:bg-primary-foreground/20 text-primary-foreground transition-all duration-200 hover:scale-110 border border-primary-foreground/20"
